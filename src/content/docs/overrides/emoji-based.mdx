...
However, when it comes to posting messages to the starboard (figuring out which settings to use for the starboard, the required amount, and appearance based settings), these type of overrides have different rules for being applied.
...
1. The first thing you need to do is add 💀 as one of the emojis used to star messages:
    ...
    <Aside type="caution">
        ...
        For example, if you have ⭐ as the server emoji, but an override for `#general` that sets the emoji to 🍣, this skullboard override won't work for messages in `#general` as the **Emoji** setting there has been overridden and doesn't include 💀.
    </Aside>
...
3.  Next, you'll need to change the settings for the override you've just created:
    ...
    You need to set the new starboard, and you have to set required stars so the override only applies after 3 💀 reactions.
    <CommandInput command="settings edit main" options={[
        {
            type: OptionType.String,
            name: 'override',
...
You have a starboard channel `#starboard` that messages go to after 3 ⭐ but you want the best starred messages to go to a separate starboard channel `#bestboard` after they get 20 ⭐.

You'll achieve this by creating an emoji override for ⭐ that's only applied after 20 reactions, that changes the **Starboard** setting.
...
3. if both overrides are emoji-based overrides, then:
